    ...
    <>
      <nav style={{
        ...
        <button className="hamburger" onClick={() => setMenuOpen(!menuOpen)}
          ...
          <span style={{ width: 22, height: 2, background: '#ddd', borderRadius: 2, transition: 'all 0.3s ease', transform: menuOpen ? 'rotate(45deg) translateY(7px)' : 'none' }} />
          <span style={{ width: 22, height: 2, background: '#ddd', borderRadius: 2, transition: 'all 0.3s ease', opacity: menuOpen ? 0 : 1 }} />
          <span style={{ width: 22, height: 2, background: '#ddd', borderRadius: 2, transition: 'all 0.3s ease', transform: menuOpen ? 'rotate(-45deg) translateY(-7px)' : 'none' }} />
        </button>
      </nav>

      <div style={{
        position: 'fixed', top: 72, right: 0, bottom: 0, width: 260,
        ...
        {LINKS.map((l) => (
          <Link key={l.to} to={l.to} onClick={() => setMenuOpen(false)} style={{
            fontSize: 16, fontWeight: 400, letterSpacing: '0.04em', textTransform: 'uppercase',
            color: current === l.to ? '#f0f0f0' : 'rgba(255,255,255,0.4)',
